...
export abstract class BaseSessionStore {
   ...
   * @returns {Promise<SessionData | null> | (SessionData | null)} A promise that resolves with the session data or `null` if not found, or the data directly if synchronous.
   */
  abstract get(
    sessionId: string
